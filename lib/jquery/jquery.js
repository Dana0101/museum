...
// Соответствует пунктирной строке для верблюда
	...
	// Используется jQuery.camelCase как обратный вызов для replace ()
...
// Текущая используемая версия jQuery
	...
	// Получение N-го элемента в наборе согласованных элементов ИЛИ
...
// Предположим, что jQuery готов без готового модуля
	...
	// parseFloat NaNs числовое приведение ложных срабатываний (null | true | false | "")
		...
		// вычитание силы бесконечности до NaN
		// добавление 1 исправляет потерю точности из parseFloat 
	...
	isPlainObject: function( obj ) {
		var key;
// Должен быть объект.
		// Из-за IE мы также должны проверить наличие свойства конструктора.
		// Убедитесь, что узлы DOM и оконные объекты также не проходят
		...
		try {
			...
			//Свойство не собственного конструктора должно быть Object
			if ( obj.constructor &&
...
// Обрабатываем итерацию по унаследованным свойствам перед собственными.
		if ( !support.ownFirst ) {
			for ( key in obj ) {
				return hasOwn.call( obj, key );
			}
		}
// Сначала перечисляются собственные свойства, поэтому для ускорения
		// если последняя собственная, то все свойства принадлежат.
		for ( key in obj ) {}
...
// Мы используем execScript в Internet Explorer
			// Мы используем анонимную функцию, поэтому контекстом является окно
			// вместо jQuery в Firefox
			( window.execScript || function( data ) {
				window["eval"].call(window, data);// jscs: игнорировать requireDotNotation
			} )( data );
		}
	},

	// Преобразуйте пунктир в camelCase; используется css и модулями данных
	...
	inArray: function( elem, arr, i ) {
		...
		if ( arr ) {
			...
			for ( ; i < len; i++ ) {
// Пропускаем доступ к разреженным массивам
	...
	merge: function( first, second ) {
		...
		// Обходной путь: преобразование .length в NaN для объектов, похожих на массивы (например, NodeLists)
		if ( len !== len ) {
...
// Связываем функцию с контекстом, при желании частично применяя любые
	// аргументы.
	...
	// Быстрая проверка, чтобы определить, является ли цель вызываемой, в спецификации
		// это вызывает TypeError, но мы просто вернем undefined.
...
// Имитация привязки
	...
	// Устанавливаем guid уникального обработчика равным оригинальному обработчику, чтобы его можно было удалить
	...
	// JSHint вызовет ошибку в этом коде из-за того, что символ не определен в ES5.
	// Определение этого глобального в .jshintrc может создать опасность использования глобального
	// без защиты в другом месте, кажется, безопаснее просто отключить JSHint для этих
	// три строки.
	/ * игнорирование jshint: начало * /
...
// проверка `in` для предотвращения ошибки JIT
	// hasOwn здесь не используется из-за ложных отрицательных значений
...
// 2. простой (захват 6)
	...
	// Ведущие и завершающие пробелы без экранирования, захват некоторых непробельных символов, предшествующих последнему
...
// NaN означает отсутствие кода
		return high !== high || escapedWhitespace ?
			escaped :
			high < 0 ?
				// Кодовая точка BMP
				String.fromCharCode( high + 0x10000 ) :
				// Дополнительная кодовая точка плоскости 
...
// Пытаемся сократить операции поиска (в отличие от фильтров) в HTML-документах
	if ( !seed ) {
		...
		if ( documentIsHTML ) {
			...
			// (кроме контекста DocumentFragment, в котором методы не существуют)
...
// селектор ID
				if ( (m = match[1]) ) {
					...
					if ( nodeType === 9 ) {
						if ( (elem = context.getElementById( m )) ) {
							...
							// getElementById может сопоставлять элементы по имени вместо 
							if ( elem.id === m ) {
								results.push( elem );
								return results;
							}
			...
			if ( support.qsa &&
				...
				// qSA смотрит за пределы контекста элемента, что нам не нужно
				// Спасибо Эндрю Дюпону за эту технику обхода
...
/**
 * Возвращает функцию для использования в псевдонимах кнопок
 * @param {String} тип
 */
 ...
 * @returns {Boolean} Истина, если элемент elem не является узлом HTML XML
 */
isXML = Sizzle.isXML = function( elem ) {
	// documentElement проверяется на случай, если он еще не существует
...
// Возврат раньше, если документ недействителен или уже выбран
	...
	// Доступ к документам iframe после выгрузки вызывает ошибку "доступ запрещен" 
	...
	support.attributes = assert(function (div) {
		div.className = "я";
		return !div.getAttribute("имя класса");
	});
	...
	// Неисправные методы getElementById не принимают программно установленные имена,
...
// поиск и фильтрация идентификаторов
	if ( support.getById ) {
...
// Узлы DocumentFragment не имеют gEBTN
	...
	// matchSelector (: active) сообщает false, если true
	rbuggyMatches = [];

	// qSa (: focus) сообщает false, когда истинно 
	...
	if ( (support.qsa = rnative.test( document.querySelectorAll )) ) {
		...
		// Стратегия Regex заимствована у Диего Перини
		assert(function( div ) {
...
// Логические атрибуты и "значение" обрабатываются неправильно
			...
			// Внутренний селектор `# id sibing-combinator selector` не работает
...
// Порядок сортировки документов
	...
	function( a, b ) {
		...
		// Рассчитываем позицию, если оба входа принадлежат одному документу
...
// Без родительских узлов либо документы, либо отключены
		...
		// Если узлы являются братьями и сестрами, мы можем быстро проверить
		} else if ( aup === bup ) {
	...
	// В противном случае нам понадобятся полные списки их предков для сравнения
		...
		// Идем по дереву в поисках несоответствия
		while ( ap[i] === bp[i] ) {
			i++;
		}
...
// При необходимости устанавливаем переменные документа
	...
	var fn = Expr.attrHandle[ name.toLowerCase() ],
		// Не обманывайтесь свойствами Object.prototype 
...
getText = Sizzle.getText = function( elem ) {
	...
	if ( !nodeType ) {
		...
		while ( (node = elem[i++]) ) {
			// Не проходить узлы комментариев
			ret += getText( node );
		}
	} else if ( nodeType === 1 || nodeType === 9 || nodeType === 11 ) {
		// Используем textContent для элементов
		// использование innerText удалено для согласованности новых строк
				...
				function( elem, context, xml ) {
					...
					if ( parent ) {
						...
						if ( simple ) {
							...
							// Обратное направление для: only- * (если мы еще не сделали этого)
						...
						if ( forward && useCache ) {
							...
							// Ищем elem по ранее кэшированному индексу

							// ... дружественным к gzip способом
							node = parent;
							outerCache = node[ expando ] || (node[ expando ] = {});

							// Защита от клонированных атрибутов 
							...
							while ( (node = ++nodeIndex && node && node[ dir ] ||
								...
								// Когда найдено, кеширует индексы на `parent` и прерывает
							...
							if ( diff === false ) {
								// Используйте тот же цикл, что и выше, для поиска элемента с самого начала
								while ( (node = ++nodeIndex && node && node[ dir ] ||
									(diff = nodeIndex = 0) || start.pop()) ) {
...
// Пользователь может использовать createPseudo, чтобы указать, что
	...
	pseudos: {
		...
		"not": markFunction(function( selector ) {
			...
			// чтобы не обрабатывать начало и конец
		...
		// или начиная с идентификатора C, сразу за которым следует "-".
...
// Разное
	...
	=
		...
		"text": function( elem ) {
			...
			// Новые значения атрибута HTML5 (например, "search") отображаются с elem.type === "text"
...
function addCombinator( matcher, combinator, base ) {
	...
	return combinator.first ?
		// Проверяем ближайшего предка / предыдущего элемента
		function( elem, context, xml ) {
			while ( (elem = elem[ dir ]) ) {
		...
		// Проверяем все предки / предшествующие элементы
		function( elem, context, xml ) {
...
// Назначить newCache, чтобы результаты распространялись на предыдущие элементы
							return (newCache[ 2 ] = oldCache[ 2 ]);
						} else {
						// Повторно использовать newcache, чтобы результаты распространялись на предыдущие элементы
							uniqueCache[ dir ] = newCache;

							// Соответствие означает, что мы закончили; неудача означает, что мы должны продолжать
...
// Применяем postFilter
		if ( postFilter ) {
			...
			// Несоответствие несоответствующих элементов, перемещая их обратно в matcherIn
			i = temp.length;
			while ( i-- ) {
				if ( (elem = temp[i]) ) {
		...
		if ( seed ) {
			if ( postFinder || preFilter ) {
				if ( postFinder ) {
					// Получаем окончательный matcherOut путем сжатия этого промежуточного звена в контексты postFinder
					temp = [];
					i = matcherOut.length;
					while ( i-- ) {
				...
				// Перемещаем согласованные элементы из исходной точки в результаты, чтобы синхронизировать их
				i = matcherOut.length;
				while ( i-- ) {
					if ( (elem = matcherOut[i]) &&
...
function matcherFromTokens( tokens ) {
	...
	for ( ; i < len; i++ ) {
		if ( (matcher = Expr.relative[ tokens[i].type ]) ) {
			...
			// Возвращаем специальный при просмотре позиционного сопоставления
			if ( matcher[ expando ] ) {
...
function matcherFromGroupMatchers( elementMatchers, setMatchers ) {
	var bySet = setMatchers.length > 0,
		...
		superMatcher = function( seed, context, xml, results, outermost ) {
			...
			// Допускать свойства NodeList, соответствующие элементы по идентификатору
			...
			// case, что приведет к "00" `matchedCount`, которое отличается от` i`, но также
			// числовой ноль.
...
compile = Sizzle.compile = function (selector, match /* Только для внутреннего использования  */ ) {
	...
	if ( !cached ) {
		...
		// Кешировать скомпилированную функцию
 ...
 * @param {Array} [seed] Набор элементов для сопоставленияt
 */
select = Sizzle.select = function( selector, context, results, seed ) {
	...
	// Пытаемся минимизировать операции, если в списке только один селектор и нет начального числа
	...
	if ( match.length === 1 ) {
		...
		if ( tokens.length > 2 && (token = tokens[0]).type === "ID" &&
			...
			// Предварительно скомпилированные сопоставители по-прежнему будут проверять происхождение, поэтому переходите на уровень 
			} else if ( compiled ) {
...
// Всегда предполагать наличие дубликатов, если они не передаются в функцию сравнения
support.detectDuplicates = !!hasDuplicate;

// Инициализируем документ по умолчанию
setDocument();

// Отделенные узлы беспорядочно следуют * друг за другом *
...
jQuery.fn.extend( {
	find: function( selector ) {
		...
		// Требуется, потому что $ (selector, context) становится $ (context) .find (selector)
...
var rootjQuery,
	...
	init = jQuery.fn.init = function( selector, context, root ) {
		...
		// init принимает альтернативный rootjQuery
		// поэтому миграция может поддерживать jQuery.sub
...
// ОБРАБОТКА: $ (html) -> $ (массив)
				if ( match[ 1 ] ) {
					...
					// скрипты верны для обратной совместимости
					// Умышленно выдача ошибки, если parseHTML не указан
...
// ОБРАБОТКА: $ (# id)
					...
					// Проверяем parentNode, чтобы поймать, когда Blackberry 4.6 вернется
					// узлы, которых больше нет в документе
					if ( elem && elem.parentNode ) {

						// Обработка случая, когда IE и Opera возвращают элементы
						// по имени вместо ID
						if ( elem.id !== match[ 2 ] ) {
							return rootjQuery.find( selector );
						}

						// В противном случае мы вставляем элемент непосредственно в объект jQuery
						this.length = 1;
						this[ 0 ] = elem;
					}
...
// Даем функции инициализации прототип jQuery для последующего создания экземпляра
...
jQuery.fn.extend( {
	...
	// Определяем положение элемента внутри
	// согласованный набор элементов
	index: function( elem ) {
...
// Удаляем дубликаты
			...
			// Обратный порядок для родительских * и пред-производных
...
// Преобразование параметров в строковом формате в параметры в объектном формате
 ...
 * список обратных вызовов ведет себя или более традиционный объект опции
 *
 ...
 * сразу после увольнения списка с последним "запомненным"
 * значения (например, Deferred)
 *
 * уникальный: гарантирует, что обратный вызов может быть добавлен только один раз (в списке нет дубликатов)
 *
 * stopOnFalse: прерывание вызова, когда обратный вызов возвращает false
 *
 */
jQuery.Callbacks = function( options ) {

	// Преобразование параметров из строкового формата в объектный при необходимости
	// (сначала проверяем кеш)
	options = typeof options === "string" ?
		createOptions( options ) :
		jQuery.extend( {}, options );

	var // Флаг, чтобы узнать, активен ли список в данный момент
		firing,
	...
	// Флаг для предотвращения стрельбы
		...
		// Очередь данных выполнения для повторяющихся списков
		queue = [],


		// Индекс текущего запущенного обратного вызова (при необходимости изменяется путем добавления / удаления)
		firingIndex = -1,

		// Запускать обратные вызовы
		fire = function() {

			// Принудительный одиночный огонь
			locked = options.once;
			...
			for ( ; queue.length; firingIndex = -1 ) {
				...
				// Запускаем обратный вызов и проверяем раннее завершение
					if ( list[ firingIndex ].apply( memory[ 0 ], memory[ 1 ] ) === false &&
						options.stopOnFalse ) {

						// Переходим к концу и забываем данные, поэтому .add не запускается повторно
						firingIndex = list.length;
						memory = false;
					}
				}
			}
// Забываем данные, если мы с ними закончили
			if ( !options.memory ) {
				memory = false;
			}
			...
			// Убираемся, если мы закончили стрельбу навсегда
			if ( locked ) {
		...
		self = {
			...
			add: function() {
				...
				// Если у нас есть память из прошлого прогона, мы должны сработать после добавления
					if ( memory && !firing ) {
			...
			remove: function() {
				jQuery.each( arguments, function( _, arg ) {
					...
					while ( ( index = jQuery.inArray( arg, list, index ) ) > -1 ) {
						...
						// Обрабатываем индексы активации
...
// Проверяем, есть ли данный обратный вызов в списке.
			// Если аргумент не задан, вернуть, есть ли у списка прикрепленные обратные вызовы.
			has: function( fn ) {
				return fn ?
		...
		// Отключить .fire и .add
			...
			// Отключить .fire
			// Также отключаем .add, если у нас нет памяти (так как это не повлияет)
			// Отменить любые ожидающие выполнения
			lock: function() {
				locked = true;
				if ( !memory ) {
					self.disable();
			...
			// Чтобы узнать, вызывались ли уже обратные вызовы хотя бы один раз
			fired: function() {
				return !!fired;
			}
		};
...
jQuery.extend( {
	...
	Deferred: function( func ) {
		var tuples = [
				...
				// действие, добавление слушателя, список слушателей, конечное состояние
				...
				then: function( /* fnDone, fnFail, fnProgress */ ) {
					...
					return jQuery.Deferred( function( newDefer ) {
						jQuery.each( tuples, function( i, tuple ) {
							...
							// отложено [сделано | сбой | progress] для перенаправления действий в newDefer
		...
		// Сохраняем трубу для обратной совместимости
		promise.pipe = promise.then;

	// Добавляем специфичные для списка методы
		jQuery.each( tuples, function( i, tuple ) {
			var list = tuple[ 2 ],
				stateString = tuple[ 3 ];

			// обещание [выполнено | сбой | прогресс] = list.add
			...
			// отложено [разрешение | отклонить | поставить в известность ]
		...
		// Вызов заданной функции, если есть
		if ( func ) {
	...
	// Отложенный помощник
	when: function (subordinate /* , ..., подчиненный */ ) {
		var i = 0,
			...
			// Мастер Deferred.
			// Если resolveValues ​​состоит только из одного Deferred, просто используйте его.
			deferred = remaining === 1 ? subordinate : jQuery.Deferred(),
// Функция обновления для значений разрешения и прогресса
		...
		// добавляем слушателей к отложенным подчиненным; относиться к другим как к решенным
		if ( length > 1 ) {
...
// Базовая функция проверки для префильтров и транспортов
...
// который принимает "плоские" варианты (не вдаваться в подробности)
...
function ajaxHandleResponses( s, jqXHR, responses ) {
	...
	// Удаляем auto dataType и получаем контент-тип в процессе
	while ( dataTypes[ 0 ] === "*" ) {
		dataTypes.shift();
		if ( ct === undefined ) {
...
// Проверяем, имеем ли мы дело с известным типом содержимого
	if ( ct ) {
	...
	if ( dataTypes[ 0 ] in responses ) {
		...
		// Пробуем конвертируемые типы данных
...
// и вернем соответствующий ответ
	if ( finalDataType ) {
...
function ajaxConvert( s, response, jqXHR, isSuccess ) {
	...
	// Работаем с копией типов данных на случай, если нам нужно изменить ее для преобразования
...
// Применяем dataFilter, если он предусмотрен
		...
		// Есть работа, только если текущий dataType не автоматический
			if ( current === "*" ) {
					...
					// Если conv2 выводит ток
						...
						if ( tmp[ 1 ] === current ) {
							...
							conv = converters[ prev + " " + tmp[ 0 ] ] ||
								converters[ "* " + tmp[ 0 ] ];
							if ( conv ) {
// Конвертеры эквивалентности конденсатора
								if ( conv === true ) {
				...
				if ( conv !== true ) {
					// Если ошибкам не разрешено всплывать, ловить и возвращать их
					if (conv && s["throws"]) { // jscs: игнорировать requireDotNotation
...
jQuery.extend( {
	...
	// Счетчик для удержания количества активных запросов
	active: 0,
	...
	ajaxSettings: {
		...
		*/
		...
		// Конвертеры данных
// Ключи разделяют исходный (или объединяющий "*") и целевой типы одним пробелом
		converters: {
...
// Разбираем текст как xml
		...
		// Для параметров, которые не следует расширять глубоко:
// вы можете добавить здесь свои собственные параметры, если
...
// Если цель не указана, записывается в ajaxSettings.
	...
	ajax: function( url, options ) {
		// Если url является объектом, имитируем подпись до 1.5
		if ( typeof url === "object" ) {
			options = url;
			url = undefined;
		}

		// Принудительно преобразовать параметры в объект
		options = options || {};

		var
			// Вары междоменного обнаружения
			parts,
			// Переменная цикла
			i,

			// URL без параметра анти-кеширования
			cacheURL,
			// Заголовки ответа в виде строки
			responseHeadersString,

			// timeout дескриптор 
			timeoutTimer,

			// Чтобы знать, должны ли отправляться глобальные события
			fireGlobals,
...
// Заголовки (отправляются все сразу)
					...
					if ( map ) {
						if ( state < 2 ) {
							for ( code in map ) {
								...
								// Ленивое добавление нового обратного вызова таким образом, чтобы сохранить старые
		...
		// Присоединяем отложенные
		deferred.promise( jqXHR ).complete = completeDeferred.add;
		jqXHR.success = jqXHR.done;
		jqXHR.error = jqXHR.fail;

		// Удаляем символ решетки 
// Добавить протокол, если он не указан
// Обработка ложного URL в объекте настроек 
// Мы также используем параметр url, если он доступен
		s.url = ( ( url || s.url || ajaxLocation ) + "" )
			.replace( rhash, "" )
			.replace( rprotocol, ajaxLocParts[ 1 ] + "//" );

		// Параметр метода псевдонима для ввода согласно тикету 
...
// Мы можем запускать глобальные события прямо сейчас, если попросим
// Не запускать события, если jQuery.event не определен в сценарии использования AMD
		...
		if ( !s.hasContent ) {
			...
			// Если данные доступны, добавить данные к url
			if ( s.data ) {
		...
		// При необходимости добавляем анти-кеш в url
			if ( s.cache === false ) {
				s.url = rts.test( cacheURL ) ?
					// Если уже есть параметр '_', установить его значение
					cacheURL.replace( rts, "$1_=" + nonce++ ) :
					// В противном случае добавляем единицу в конец
	...
	// Устанавливаем заголовок Accepts для сервера в зависимости от dataType
		...
		// Проверяем опцию заголовков
		for ( i in s.headers ) {
			jqXHR.setRequestHeader( i, s.headers[ i ] );
		}

		// Разрешить настраиваемые заголовки / миметипы и преждевременное прерывание
		if ( s.beforeSend &&
			( s.beforeSend.call( callbackContext, jqXHR, s ) === false || state === 2 ) ) {

			// Отмена, если еще не сделано, и возврат
			return jqXHR.abort();
		}
		...
		// Устанавливаем обратные вызовы для отложенных
...
// Отправляем глобальное событие
			...
			// Распространение исключения как ошибки, если не сделано
				if ( state < 2 ) {
					done( -1, e );
...
// (независимо от того, как долго будет использоваться объект jqXHR)
			...
			// Кешировать заголовки ответа
			responseHeadersString = headers || "";

			// Устанавливаем readyState
		...
		// Получаем данные ответа
			...
			// Преобразование независимо от того, что (таким образом поля responseXXX всегда устанавливаются)
			...
			// если нет содержимого
				...
				// Если у нас есть данные, давайте их конвертируем
				} else {
				...
				// Извлекаем ошибку из statusText
// затем нормализуем statusText и status для неотменяемых
				error = statusText;
...
jQuery.each( [ "get", "post" ], function( i, method ) {
	jQuery[ method ] = function( url, data, callback, type ) {
		// сдвигаем аргументы, если аргумент данных был опущен
		...
		// URL-адрес может быть объектом параметров (который тогда должен иметь .url)
...
jQuery._evalUrl = function( url ) {
	...
	// Сделайте это явным, так как пользователь может переопределить это через ajaxSetup
		type: "GET",
		dataType: "script",
		cache: true,
		async: false,
...
function filterHidden( elem ) {
	...
	// Отсоединенные элементы считаются скрытыми
	...
	if ( jQuery.isArray( obj ) ) {
		...
		jQuery.each( obj, function( i, v ) {
			if ( traditional || rbracket.test( prefix ) ) {
				...
				// Элемент не скалярный (массив или объект), закодируйте его в числовой индекс.
				buildParams(
...
// ключи / значения в строку запроса
jQuery.param = function( a, traditional ) {
	var prefix,
		s = [],
		add = function( key, value ) {

			// Если значение является функцией, вызвать ее и вернуть ее значение
			value = jQuery.isFunction( value ) ? value() : ( value == null ? "" : value );
			s[ s.length ] = encodeURIComponent( key ) + "=" + encodeURIComponent( value );
		};

	// Устанавливаем традиционное значение true для поведения jQuery <= 1.3.2.
	...
	// Если традиционно, кодировать "старым" способом (способ 1.3.2 или старше
...
// (Это все еще прикреплено к ajaxSettings для обратной совместимости)
jQuery.ajaxSettings.xhr = window.ActiveXObject !== undefined ?


	function() {

		// XHR не может получить доступ к локальным файлам, в этом случае всегда используйте ActiveX
		if ( this.isLocal ) {
...
// Для всех остальных браузеров используйте стандартный объект XMLHttpRequest
	createStandardXHR;
...
// Открытые запросы должны прерываться вручную при выгрузке
...
// Определение опорных свойств
support.cors = !!xhrSupported && ( "withCredentials" in xhrSupported );
xhrSupported = support.ajax = !!xhrSupported;
// Создаем транспорт, если браузер может предоставить xhr
if ( xhrSupported ) {

	jQuery.ajaxTransport( function( options ) {

		// Междоменный доступ разрешен, только если поддерживается через XMLHttpRequest
...
// Применение настраиваемых полей, если они есть
...
// Для междоменных запросов, учитывая условия предварительной проверки
// что-то вроде головоломки, мы просто никогда не ставили ее точно.
// (его всегда можно установить для каждого запроса или даже с помощью ajaxSetup)
// Для запросов к тому же домену заголовок не изменится, если он уже указан.
					...
					for ( i in headers ) {
						...
						// IE ActiveXObject выдает исключение "Несоответствие типов" при установке
// запрос заголовка нулевого значения.
//
// Чтобы сохранить согласованность с другими реализациями XHR, приведите значение
// в строку и игнорировать `undefined`.
...
// обрабатывается в jQuery.ajax (поэтому здесь не нужно пытаться / поймать)
					xhr.send( ( options.hasContent && options.data ) || null );
					// Слушатель
					callback = function( _, isAbort ) {
						var status, statusText, responses;
						// Никогда не вызывалась и прерывается или завершается
						if ( callback && ( isAbort || xhr.readyState === 4 ) ) {
							// Очистить
							delete xhrCallbacks[ id ];
							callback = undefined;
							xhr.onreadystatechange = jQuery.noop;
// Отменить вручную, если необходимо
							if ( isAbort ) {
								if ( xhr.readyState !== 4 ) {
									xhr.abort();
								}
								...
								// Доступ к двоичным данным responseText вызывает исключение
								if ( typeof xhr.responseText === "string" ) {
									responses.text = xhr.responseText;
								}
								// Firefox выдает исключение при доступе к
// statusText для ошибочных междоменных запросов
								try {
									statusText = xhr.statusText;
								} catch ( e ) {
									// Нормализуем с помощью Webkit, давая пустой statusText
									statusText = "";
								}

								// Статус фильтра для нестандартного поведения

// Если запрос локальный и у нас есть данные: предполагаем успех
// (успех без данных не получит уведомления, это лучшее, что мы
// можно делать с учетом текущих реализаций)
						...
						// Вызов завершен, если необходимо
						if ( responses ) {
							complete( status, statusText, responses, xhr.getAllResponseHeaders() );
						}
					};
// Отправляем запрос
// `xhr.send` может вызвать исключение, но оно будет
// обрабатывается в jQuery.ajax (поэтому здесь не нужно пытаться / поймать)
					if ( !options.async ) {

						// Если мы находимся в режиме синхронизации, мы запускаем обратный вызов
						callback();
					} else if ( xhr.readyState === 4 ) {

						// получено напрямую, нам нужно запустить обратный вызов
						window.setTimeout( callback );
					} else {

					// Регистрируем обратный вызов, но задерживаем его, если `xhr.send` выдает
// Добавляем в список активных обратных вызовов xhr
...
// Обрабатываем особый случай кеша и глобальный
...
// Связываем тег скрипта hack transport
jQuery.ajaxTransport( "script", function( s ) {

	// Этот транспорт работает только с междоменными запросами
	if ( s.crossDomain ) {
		...
		return {
			...
			send: function( _, callback ) {
				...
				// Присоединяем обработчики для всех браузеров
				script.onload = script.onreadystatechange = function( _, isAbort ) {
					...
					if ( isAbort || !script.readyState || /loaded|complete/.test( script.readyState ) ) {
						...
						// Разыменовать скрипт
						script = null;
						// Обратный вызов, если не прервать
						if ( !isAbort ) {
...
jQuery.ajaxPrefilter( "json jsonp", function( s, originalSettings, jqXHR ) {
	...
	if ( jsonProp || s.dataTypes[ 0 ] === "jsonp" ) {
		...
		// Получаем имя обратного вызова, запоминая уже существующее значение, связанное с ним
		...
		// принудительно использовать json dataType
		s.dataTypes[ 0 ] = "json";
...
// Если предыдущего значения не существовало - удаляем его
			if ( overwritten === undefined ) {
				jQuery( window ).removeProp( callbackName );

			// В противном случае восстанавливаем существующее значение
			} else {
				window[ callbackName ] = overwritten;
			}

			// Сохранить как бесплатную
			if ( s[ callbackName ] ) {

				// убедитесь, что повторное использование опций не напортачит
...
// keepScripts (необязательно): если true, будут включены скрипты, переданные в строку html
...
// Сохраняем копию старого метода загрузки
var _load = jQuery.fn.load;

/**
	* Загрузить URL - адрес на страницу
		*/
...
// Если это функция
	...
	// Если у нас есть элементы, которые нужно изменить, делаем запрос
	if ( self.length > 0 ) {
		jQuery.ajax( {
			url: url,

			// Если переменная типа не определена, будет использован метод GET.
// Сделаем значение этого поля явным, поскольку
// пользователь может переопределить его через метод ajaxSetup
			type: type || "GET",
			dataType: "html",
			data: params
			...
			self.html( selector ?
				...
				// Если был указан селектор, размещаем нужные элементы в фиктивном div
// Исключаем скрипты, чтобы избежать ошибок IE 'Permission Denied'
				jQuery( "<div>" ).append( jQuery.parseHTML( responseText ) ).find( selector ) :

				// В противном случае использовать полный результат
				responseText );

		// Если запрос завершился успешно, эта функция получает «данные», «статус», «jqXHR»
// но они игнорируются, потому что ответ был указан выше.
...
jQuery.offset = {
	setOffset: function( elem, options, i ) {
		...
		// сначала устанавливаем позицию, в случае, если верх / лево установлены даже на статическом элементе
		if ( position === "static" ) {
		...
		// нужно иметь возможность вычислять позицию сверху или слева
// автоматически, а положение либо абсолютное, либо фиксированное
		...
		if ( jQuery.isFunction( options ) ) {
			...
			// Используйте jQuery.extend здесь, чтобы разрешить изменение аргумента координат 
...
jQuery.fn.extend( {
	...
	// Убедитесь, что это не отключенный узел DOM
		if ( !jQuery.contains( docElem, elem ) ) {
			return box;
		}

	// Если у нас нет gBCR, просто используйте 0,0 вместо ошибки
	...
	position: function() {
		...
		// Фиксированные элементы смещены от окна (родительское смещение = {top: 0, left: 0}, 
		// потому что это единственный родительский элемент смещения
		if ( jQuery.css( elem, "position" ) === "fixed" ) {

			// мы предполагаем, что getBoundingClientRect доступен, когда вычисляемая позиция фиксирована
			offset = elem.getBoundingClientRect();
		} else {
			// Получение * реального * offsetParent
			offsetParent = this.offsetParent();

			// Получаем правильные смещения
			offset = this.offset();
			if ( !jQuery.nodeName( offsetParent[ 0 ], "html" ) ) {
				parentOffset = offsetParent.offset();
			}

			// Добавляем границы offsetParent
			parentOffset.top  += jQuery.css( offsetParent[ 0 ], "borderTopWidth", true );
			parentOffset.left += jQuery.css( offsetParent[ 0 ], "borderLeftWidth", true );
		}

		// Вычесть родительские смещения и поля элемента
		// Примечание: если у элемента есть margin: auto, смещение Left и margin Left
		// одинаковы в Safari, что приводит к неправильному значению offset.left 0
		return {
...
// Добавляем верхние / левые cssHooks, используя jQuery.fn.position
// getComputedStyle возвращает процент, если указано для верхнего / левого / нижнего / правого
// вместо того, чтобы делать модуль css зависимым от модуля смещения, мы просто проверяем его здесь
...
jQuery.each( { Height: "height", Width: "width" }, function( name, type ) {
	...
	// маржа только для outerHeight, outerWidth
...
// Количество элементов, содержащихся в согласованном наборе элементов
...
// файлы, которые могут использовать определение, но не через соответствующий скрипт конкатенации, который
// понимает анонимные модули AMD. Названный AMD - самый безопасный и надежный
// способ регистрации. Строчный jquery используется, потому что имена модулей AMD
// получено из имен файлов, и jQuery обычно доставляется в нижнем регистре
// имя файла. Сделайте это после создания глобального, чтобы, если модуль AMD хочет
...
// объявляем себя анонимными модулями и избегаем установки глобального, если
...
var
	...
	// Отображение над $ в случае перезаписи
	_$ = window.$;
